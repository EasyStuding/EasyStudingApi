...
namespace EasyStudingUnitTests.RepositoryTests
{
    public class StateRepositoryTest
    {
        ...
        [Fact(DisplayName = "StateRepository.Edit(7) should return index out of range exception.")]
        public async void StateRepository_Edit_7_should_return_index_out_of_range_exception()
        {
            using (Context = new TestDbContext().Context)
            {
                var rep = new StateRepository(Context);
                var ex = await Assert.ThrowsAsync<IndexOutOfRangeException>(async () => await rep.Edit(new State() { Id = 7 }));
        ...
        [Fact(DisplayName = "StateRepository.Remove(7) should return index out of range exception.")]
        public async void StateRepository_Remove_7_should_return_index_out_of_range_exception()
        {
            using (Context = new TestDbContext().Context)
            {
                var rep = new StateRepository(Context);
                var ex = await Assert.ThrowsAsync<IndexOutOfRangeException>(async () => await rep.Remove(new State() { Id = 7 }));
