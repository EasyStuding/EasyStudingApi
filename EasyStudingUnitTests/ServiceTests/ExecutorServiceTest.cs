...
namespace EasyStudingUnitTests.ServiceTests
{
    public class ExecutorServiceTest
    {
        ...
        public async void ExecutorService_GetOrders_valid_should_return_3_objects()
        {
            var service = new ExecutorService(new TestUserRepository(), new TestOrderRepository(), new TestSkillRepository(), new TestUserSkillRepository());

            var result = await service.GetOrders("High", "Westeros", "Brest", "Winteful", 1);
